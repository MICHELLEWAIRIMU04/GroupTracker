...
export async function sendGroupInviteEmail(email, groupName, inviterName, token, isAdmin = false) {
  ...
  const mailOptions = {
    from: process.env.SMTP_FROM || process.env.SMTP_USER,
    to: email,
    subject: `You're invited to join "${groupName}" - Group Activity Tracker`,
    html: `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            <h2>Hi there!</h2>
            <p><strong>${inviterName}</strong> has invited you to join their group on Group Activity Tracker!</p>
            ...
            <p>Group Activity Tracker helps teams collaborate and track contributions together. You'll be able to:</p>
            <ul>
